...
apt-get update 
apt-get install -y  build-essential python-setuptools python-software-properties python-dev libmysqlclient-dev libxml2-dev libxslt1-dev git
easy_install pip 
pip install tox
...
pushd /src/trove/
su - vagrant -c "cp -r /src/trove ~/; pushd ~/trove/; tox" 